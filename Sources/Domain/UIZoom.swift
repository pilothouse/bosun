...
/// whole **percent** on a fixed 5% grid in `[50, 200]` (an `Int`, so it never drifts the way a
...
public struct UIZoom: Equatable, Sendable, Codable {
    ...
    /// The grid the in/out steps move on (5%), also used to snap a stray restored value.
