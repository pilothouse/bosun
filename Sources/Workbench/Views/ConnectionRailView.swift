...
final class ConnectionRailView: FlippedView {
    ...
    private func connRow(_ c: Connection, width: CGFloat, filledStar: Bool, t: Theme) -> ClickRow {
        ...
        let star = label(filledStar ? "★" : "☆", sys(12), filledStar ? t.accent : t.txt5)
        star.frame = NSRect(x: width - 24, y: 13, width: 14, height: 14)
        row.addSubview(star)
        return row
    }
    ...
    private func rebuild() {
        ...
        plus.frame = NSRect(x: 14, y: 11, width: 26, height: 26)
        let pl = label("+", sys(16), t.txt4, align: .center)
        pl.frame = plus.bounds; plus.addSubview(pl); footer.addSubview(plus)
        let ftLabel = label("New connection", sys(11.5), t.txt3)
        ftLabel.frame = NSRect(x: 49, y: 14, width: w - 49 - 40, height: 18); footer.addSubview(ftLabel)
        let cmdN = label("⌘N", mono(10), t.txt5, align: .right)
        cmdN.frame = NSRect(x: w - 44, y: 14, width: 30, height: 18); footer.addSubview(cmdN)
        addSubview(footer)
        ...
        func section(_ title: String, _ items: [Connection], star: Bool, filled: Bool, count: String?) {
            let head = sectionHeader(title, accentStar: star, count: count, width: w, t: t)
            head.frame.origin.y = y; doc.addSubview(head); y += 28
            for c in items {
                let r = connRow(c, width: w, filledStar: filled, t: t)
                r.frame.origin.y = y; doc.addSubview(r); y += 42
            }
            y += 8
        }
        section("FAVORITES", Mock.favorites, star: true, filled: true, count: "\(Mock.favorites.count)")
        section("SSH REMOTES · VIA TAILSCALE", Mock.sshRemotes, star: false, filled: false, count: nil)
        section("LOCAL FOLDERS", Mock.folders, star: false, filled: false, count: nil)
